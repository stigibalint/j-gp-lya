...
function placeYellowDivs() {
    for (let i = 0; i < numberOfYellowDivs; i++) {
        ...
        yellowDiv.addEventListener('click', () => {
            removeGreenDivFromCell(cells[randomIndex]);
        });
...
function moveGreenDivs() {
    ...
    greenDivs.forEach(greenDiv => {
        ...
        if (possibleMoves.length > 0) {
            ...
            cells[randomMove].appendChild(greenDiv);
        }
    });
}

function removeGreenDivFromCell(cell) {
    const greenDiv = cell.querySelector('.green');
    if (greenDiv) {
        greenDiv.remove();
    }
}
